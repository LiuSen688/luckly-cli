<template>
  <div class="varlet-site-sidebar var-elevation--3">
    ...
    >
      ...
      <span class="varlet-site-sidebar__item--title" v-if="item.type === menuTypes.TITLE">{{ item.text[language as string]
        }}</span>
      <span v-else>{{ item.text[language as string] }}</span>
    </var-cell>
  </div>
</template>
